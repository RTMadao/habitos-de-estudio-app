                                    ...
                                    <v-flex px-2 xs11 style="max-height:30px" v-for="(tarea,index) in tareas" v-bind:key="index">
                                        <v-flex v-if="tarea.hecha" >
                                            <v-flex v-if="tarea.verificada">
                                                <div style="color:grey; text-decoration:line-through;"> [{{tarea.fecha}} ] {{tarea.descripcion}} en {{tarea.materia}} </div>
                                            </v-flex>
                                            <v-flex v-else>
                                                <div style="color:grey;"> [{{tarea.fecha}} ] {{tarea.descripcion}} en {{tarea.materia}} (Revisar) </div>
                                            </v-flex>
                                        </v-flex>
                                        <v-flex v-else>
                                            <div style="color:black;"> [{{tarea.fecha}} ] {{tarea.descripcion}} en {{tarea.materia}} </div>
                                        </v-flex>
                                    </v-flex>
                                </v-layout>
                            </v-card> 
                ...
                <v-layout xs10 mt-5 class="formTarea" justify-center align-center>
                    <v-card style="width:450px; height:320px" color="yellow">
                        <v-layout xs8 justify-center align-center class="formcontainer"  column>
                            ...
                            <v-flex mt-3> <textarea v-model="tareaAgregar.descripcion" rows="3" cols="23" style="background-color:white; border-radius: 4px 4px 4px 4px; -moz-border-radius: 4px 4px 4px 4px; -webkit-border-radius: 4px 4px 4px 4px; border: 1px solid darkgrey" placeholder="Descripcion"> </textarea> </v-flex>
